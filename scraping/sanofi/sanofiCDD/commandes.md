...
```sh
gcloud builds submit --tag gcr.io/projetdbt-450020/scraper-job-sanoficddoffre
```
...
```sh
gcloud run jobs create scraper-job-sanoficddoffre \
    --image=gcr.io/projetdbt-450020/scraper-job-sanoficddoffre \
    --region=us-central1 \
    --task-timeout=1800s \
    --max-retries=3 \
    --set-cloudsql-instances=projetdbt-450020:us-central1:projetcddnterne \
    --set-env-vars DB_HOST="/cloudsql/projetdbt-450020:us-central1:projetcddnterne",DB_NAME="postgres",DB_USER="postgres",DB_PASSWORD="root"
```
