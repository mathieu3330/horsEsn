...
```sh
gcloud builds submit --tag gcr.io/horsesn/scraper-job-sanoficddoffre
```
...
gcloud run jobs create scraper-job-sanoficddoffre \
    --image=gcr.io/horsesn/scraper-job-sanoficddoffre \
    --region=us-central1 \
    --task-timeout=1800s \
    --max-retries=3 \
    --set-cloudsql-instances=horsesn:us-central1:projetcddnterne \
    --set-env-vars DB_HOST="/cloudsql/horsesn:us-central1:projetcddnterne",DB_NAME="postgres",DB_USER="postgres",DB_PASSWORD="root"
```
