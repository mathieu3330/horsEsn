...
```sh
gcloud builds submit --tag gcr.io/projetdbt-450020/scraper-job-safran
```
...
```sh
gcloud run jobs create scraper-job-safran \
    --image=gcr.io/projetdbt-450020/scraper-job-safran \
    --region=us-central1 \
    --task-timeout=1800s \
    --max-retries=3 \
