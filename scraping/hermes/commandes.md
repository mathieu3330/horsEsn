...
```sh
gcloud builds submit --tag gcr.io/projetdbt-450020/scraper-job-hermes
```
...
```sh
gcloud run jobs create scraper-job-hermes \
    --image=gcr.io/projetdbt-450020/scraper-job-hermes \
    --region=us-central1 \
    --task-timeout=1800s \
    --max-retries=3 \
