...
```sh
gcloud builds submit --tag gcr.io/projetdbt-450020/scraper-job-sncf
```
...
```sh
gcloud run jobs create scraper-job-sncf \
    --image=gcr.io/projetdbt-450020/scraper-job-sncf \
    --region=us-central1 \
    --task-timeout=1800s \
    --max-retries=3 \
...
```sh
gcloud run jobs execute scraper-job-credit-sncfOffre --region=us-central1
```
